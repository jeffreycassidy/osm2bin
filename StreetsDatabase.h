...
public:
	...
	const PathNetwork& roads() 					const { return m_roadNetwork; }
	const std::vector<std::string>& streets() 	const { return m_streets; }
	...
	const POI& poi(unsigned poiID) const { return m_pois.at(poiID); }
	unsigned getNumberOfPOIs() const { return m_pois.size(); }

	const Feature& feature(unsigned featureID) const { return m_features.at(featureID); }
	unsigned getNumberOfFeatures() const { return m_features.size(); }

	void pois(std::vector<POI>&& p){ m_pois=std::move(p); }
	void features(std::vector<Feature>&& f){ m_features=std::move(f); }
