...
public:
	...
	unsigned 	pointCount() 		const { return m_points.size(); }
	LatLon 		point(unsigned i) 	const { return m_points.at(i); }

	const std::vector<LatLon>& points() const { return m_points; }

	std::pair<OSMID,OSMEntityType>	id() const { return std::make_pair(m_id,m_osmType); }

	FeatureType type() 				const { return m_type; }

	bool bounded() 					const { return m_bounded;	} // true if does not go out of bounds

	bool isWater()					const { return m_type==Lake || m_type==River; }
...
private:
	...
	template<class Archive>void serialize(Archive& ar,const unsigned ver){ ar & m_id & m_name & m_points & m_type & m_osmType; }
