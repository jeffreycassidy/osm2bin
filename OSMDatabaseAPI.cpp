...
unsigned long long getNumberOfNodes()		{ return osmdb.nodes().size(); }
unsigned long long getNumberOfWays()		{ return osmdb.ways().size(); }
unsigned long long getNumberOfRelations()	{ return osmdb.relations().size(); }

// Query all nodes in the database, by node index
const OSMNode*          getNodeByIndex(unsigned idx)		{ return &osmdb.nodes().at(idx); }
const OSMWay*           getWayByIndex(unsigned idx)			{ return &osmdb.ways().at(idx); }
const OSMRelation*      getRelationByIndex(unsigned idx)	{ return &osmdb.relations().at(idx); }
